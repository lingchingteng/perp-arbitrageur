...
export class PerpService {
  ...
  private async createAmmReader(): Promise<AmmReader> {
    return this.createContract<AmmReader>(
      (systemMetadata) => systemMetadata.ammReaderAddr,
      AmmReaderArtifact.abi
    );
  }

  private async createClearingHouse(
  ...
  private async createClearingHouseViewer(
    signer?: ethers.Signer
  ...
  private async createContract<T>(
    ...
    return this.ethService.createContract<T>(
      addressGetter(systemMetadata),
      abi,
      signer
    );
  }
  ...
  async getPosition(ammAddr: string, traderAddr: string): Promise<Position> {
    ...
    const position = (
      await clearingHouse.functions.getPosition(ammAddr, traderAddr)
    )[0];
    return {
      ...
      lastUpdatedCumulativePremiumFraction: PerpService.fromWei(
        position.lastUpdatedCumulativePremiumFraction.d
      ),
    };
  }
  ...
  async getMarginRatio(ammAddr: string, traderAddr: string): Promise<Big> {
    const clearingHouse = await this.createClearingHouse();
    return PerpService.fromWei(
      (await clearingHouse.functions.getMarginRatio(ammAddr, traderAddr))[0].d
    );
  }
  ...
  async getPositionNotionalAndUnrealizedPnl(
    ...
    const clearingHouse = await this.createClearingHouse();
    const ret = await clearingHouse.getPositionNotionalAndUnrealizedPnl(
      ammAddr,
      traderAddr,
      pnlCalcOption
    );
  ...
  async getUnrealizedPnl(
    ammAddr: string,
    traderAddr: string,
    pnlCalOption: PnlCalcOption
  ): Promise<Big> {
    const clearingHouseViewer = await this.createClearingHouseViewer();
    const unrealizedPnl = (
      await clearingHouseViewer.functions.getUnrealizedPnl(
        ammAddr,
        traderAddr,
        BigNumber.from(pnlCalOption)
      )
    )[0];
